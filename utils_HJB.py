...
#         return -np.maximum(1-self.St**2,0)**2
    ...
    def solve(self, U0):
        ...
        for n in range(1, self.N):
            t = n * self.dt
            U = U - self.dt * np.maximum(D_minus @ U + q_minus,
                                         -D_plus @ U - q_plus)
    ...
    def solve_1(self, U0):
        ...
        for n in range(1, self.N):
            U = U - self.dt * np.maximum(self.c * D_minus @ U,
                                         -self.c * D_plus @ U)
        return U
    ...
    def RK2_solve(self, U0):
        ...
        for n in range(1, self.N):
            U1 = U - self.dt * np.maximum(self.c * D_minus @ U,
                                          -self.c * D_plus @ U)
            U2 = U1 - self.dt * np.maximum(self.c * D_minus @ U1,
                                           -self.c * D_plus @ U1)
            U = (U + U2) / 2
        return U
    ...
    def point_S(self, U, S):
        ...
        if self.St[i] == S:
            ...
            U_s = ((self.St[i] - S) * U[i - 1] +
                   (S - self.St[i - 1]) * U[i]) / self.h
        return U_s
...
class Eikonal_IE:
    ...
    def newton(self, x, B, b, C, c, eta, k_max):
        ...
        while (errs[-1] > eta and k < k_max):
            ...
            x_new = x - np.linalg.inv(F_prime) @ np.maximum(
                B @ x - b, C @ x - c)
    ...
    def solve(self, U0, schema="newton"):
        ...
        if schema == "newton":
            for t in range(self.N):
                ...
                U, errs = self.newton(U, B_plus, b_plus, B_minus, b_minus,
                                      0.0001, 100)
...
def evaluation(Is, Ns, T=1, S_max=3, S_min=-3, monotone=True, RK2=False):
    ...
    for i in range(n - 2):
        alpha_k[i] = np.round(compute_alpha(ek[i], ek[i + 1], h[i], h[i + 1]),
                              6)
...
def table(Is, Ns, U_s, e_k, order_alpha_k, schema, caption="N=I"):
    ...
    df['order alpha_k'] = np.concatenate((['-',
                                           '-'], np.round(order_alpha_k, 6)))
    styles = [
        dict(selector="caption",
             props=[("text-align", "center"), ("font-size", "110%"),
                    ("color", 'black')])
    ]
    caption = schema + ' (' + caption + ')'
    df_styler = df.style.set_table_attributes(
        "style='display:inline'").set_caption(caption).set_table_styles(styles)
    return df_styler
